...
y_onehot = []
# Read image 00
print("<Data load set 03>")
img_list = glob.glob('./learning_data_3/camera_im/*.jpeg')
img_list_sorted = natsorted(img_list,reverse=False)  # Sort the images / 텍스트로 이루어진 숫자 정렬
...
# Read Value 00
f = open('./learning_data_3/textfile.txt', 'r')
...
for pos in y_data:
    ...
    y_onehot.append(one_hot(tmp + 10,31)) # ex) 0 -> [1,0,0...], 1 -> [0,1,0...]
...
class Model:
    ...
    def _set_param(self):
        ...
        self.Y = tf.placeholder(tf.float32, [None, 31])
    ...
    def _build_net(self): #생성자에서 기본 실행, 신경망 구성
        with tf.variable_scope(self.name):
            ...
            self.FC = tf.layers.dense(inputs=self.FC, units=31, kernel_initializer=tf.contrib.layers.xavier_initializer())
