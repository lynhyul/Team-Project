...
class Model:
    ...
    def _set_param(self):
        ...
        self.Y = tf.placeholder(tf.float32, [None, 41])
    ...
    def _build_net(self): #생성자에서 기본 실행, 신경망 구성
        with tf.variable_scope(self.name):
            ...
            self.FC = tf.layers.dense(inputs=self.FC, units=41, kernel_initializer=tf.contrib.layers.xavier_initializer())
...
def lane(data):
    ...
    # find lane with NN
    mid_line_onehot = m1.operator([img_overhead])
    mid_line_imm = np.argmax(mid_line_onehot,1)
    mid_line = 10 * (mid_line_imm - 10)
